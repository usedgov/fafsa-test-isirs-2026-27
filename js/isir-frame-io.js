...
export async function * aiter_read_isir_frames(filelist_src) {
    ...
    const rx_fake_test_isir_leader = /7FAKE-UUID-FROM-TEST-INPUT/;
    ...
    function select_isir_frame_substring(ln) {
        ...
        if(idx < 0){
            idx = ln.search(rx_fake_test_isir_leader);
        }
        return (idx>=0 ?
            ln.slice(idx, idx+7944) // 2026-2027 ISIR Record Layout Total Length
